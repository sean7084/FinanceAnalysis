...
| Fundamental Snapshot: PE | TuShare `daily_basic` | `factors_fundamentalfactorsnapshot.pe` | `1,099,979` non-null rows, `2001-07-24` to `2026-04-22` | Stored as a secondary legacy valuation field for audit/backward-compatibility; active downstream scoring now prefers `pe_ttm`. | Missing rows mainly affect legacy comparison surfaces after the `pe_ttm` cutover. |
| Fundamental Snapshot: PE TTM | TuShare `daily_basic` | `factors_fundamentalfactorsnapshot.pe_ttm` | Additive field introduced for the PE TTM cutover; populate via `backfill_fundamental_snapshots` after migration. | Primary valuation input for `FactorScore.pe_ttm_percentile_score`, then direct `LightGBM`/`LSTM` valuation features and API/dashboard factor surfaces. | Missing rows push the active valuation-derived features toward neutral scores on affected asset-dates. |
...
| Factor Score: PE TTM Percentile | Percentile rank from PE TTM history | `factors_factorscore.pe_ttm_percentile_score` | Additive field introduced for the PE TTM cutover; recompute via factor-score refresh/backfill after migration. | Primary valuation feature for `LightGBM`/`LSTM`; indirect `Heuristic` input through composite/bottom-probability scores; API/dashboard component surface. | Gaps weaken active valuation context and push affected model features toward neutral `0.5`. |
...
| Prediction Result Rows | Generic daily prediction snapshot storage | `prediction_predictionresult` | `10,804` rows, `300` assets, `2024-12-31` to `2026-04-27` | Current heuristic/LSTM stock-level and batch prediction APIs plus dashboard latest-row joins; backtests regenerate heuristic and `LSTM` candidates on demand. | Missing rows reduce current snapshot API coverage but do not block runtime backtest candidate generation. |
| LightGBM Prediction Rows | Daily LightGBM snapshot storage | `prediction_lightgbmprediction` | `446,482` rows, `300` assets, `2016-06-30` to `2026-04-27` | Current stock-level/batch `LightGBM` prediction APIs plus dashboard latest-row joins; backtests regenerate `LightGBM` candidates on demand from active artifacts. | Missing stored rows reduce current snapshot API coverage but do not block runtime inference while active artifacts exist. |
...
Historical model inputs are still materialized for training, validation, and inspection.
Stored prediction tables remain only as the live daily snapshot layer used by current stock-level prediction APIs and dashboard latest-row joins.
Legacy paginated prediction-history browsing is no longer treated as a supported backtest or monitoring dependency.
