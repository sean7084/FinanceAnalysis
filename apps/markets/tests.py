...
class IndexConstituentSyncTests(TestCase):
        ...
        class StubPro:
            def stock_basic(self, **kwargs):
                if kwargs['list_status'] == 'L':
                    return pd.DataFrame([
                        {'ts_code': '600001.SH', 'symbol': '600001', 'name': 'Existing Member Asset', 'list_date': '20100105', 'list_status': 'L'},
                        {'ts_code': '600002.SH', 'symbol': '600002', 'name': 'New Member Asset', 'list_date': '20120608', 'list_status': 'L'},
                    ])
                return pd.DataFrame([])
    ...
    def test_sync_index_constituents_persists_membership_tags_and_dedupes_dispatch(self, mock_pro_api, mock_delay):
        ...
        new_member_asset = Asset.objects.get(ts_code='600002.SH')

        self.assertCountEqual(self.existing_asset.membership_tags, ['CSI500'])
        self.assertCountEqual(new_member_asset.membership_tags, ['CSI500'])

        self.assertEqual(IndexMembership.objects.filter(asset=self.existing_asset, trade_date='2026-04-25').count(), 1)
        self.assertEqual(IndexMembership.objects.filter(index_code='000905.SH').count(), 3)

        self.assertEqual(mock_delay.call_count, 2)
        dispatched_symbols = {call.args[0] for call in mock_delay.call_args_list}
        self.assertEqual(dispatched_symbols, {'600001', '600002'})

        rendered_output = output.getvalue()
        self.assertIn('current_union_count=2', rendered_output)
        self.assertIn('overlap_count=0', rendered_output)
        self.assertIn('new_assets=1', rendered_output)
        self.assertIn('dispatched_assets=2', rendered_output)
    ...
    def test_sync_index_constituent_universe_chunks_large_index_weight_ranges(self, mock_pro_api):
        ...
        self.assertEqual(
            ...
            [
                ('000905.SH', '20260101', '20260102'),
                ('000905.SH', '20260103', '20260104'),
                ('000905.SH', '20260105', '20260105'),
            ],
        )
    ...
    def test_sync_index_constituent_universe_retries_tushare_rate_limit(self, mock_pro_api, mock_sleep):
        ...
        self.assertEqual(summary['latest_trade_dates']['000905.SH'], '20260104')
    ...
    def test_sync_daily_a_shares_queues_post_sync_refresh_after_asset_syncs(
        ...
        mock_benchmark_sync.return_value = {
            'latest_trade_dates': {'000905.SH': '20260425'},
            'rows_written': 3,
        }
        ...
        mock_sync_universe.return_value = {
            ...
            'overlap_count': 0,
            'current_constituent_counts': {'000905.SH': 3},
    ...
    def test_sync_daily_a_shares_adds_warmup_repairs_for_new_pre_floor_union_assets(
        ...
        mock_sync_universe.return_value = {
            ...
            'overlap_count': 0,
            'current_constituent_counts': {'000905.SH': 1},
    ...
    def test_sync_index_constituents_can_dispatch_only_current_membership_changes(self, mock_pro_api, mock_delay):
        ...
        Asset.objects.create(
            ...
            symbol='600002',
            ts_code='600002.SH',
            name='Existing Member Asset',
            membership_tags=['CSI500'],
        )
        ...
        self.assertEqual(summary['dispatched_assets'], 1)
        dispatched_symbols = {call.args[0] for call in mock_delay.call_args_list}
        self.assertEqual(dispatched_symbols, {'600001'})
        self.assertCountEqual(self.existing_asset.membership_tags, ['CSI500'])
    ...
    def test_sync_monthly_index_memberships_uses_change_only_dispatch(self, mock_sync):
        ...
        self.assertEqual(mock_sync.call_args.kwargs['index_codes'], ('000905.SH',))
    ...
    def test_sync_benchmark_index_history_persists_rows(self, mock_pro_api):
        ...
        self.assertEqual(summary['index_codes'], ['000905.SH'])
        self.assertEqual(BenchmarkIndexDaily.objects.filter(index_code='000905.SH').count(), 2)
        self.assertEqual(
            BenchmarkIndexDaily.objects.get(index_code='000905.SH', trade_date='2026-04-25').close,
            Decimal('3940.0'),
        )
...
class PointInTimeUniverseResolutionTests(TestCase):
    def setUp(self):
        self.sse = Market.objects.create(code='SSE', name='Shanghai Stock Exchange')
        self.szse = Market.objects.create(code='SZSE', name='Shenzhen Stock Exchange')
        self.member_asset = Asset.objects.create(
            market=self.sse,
            symbol='600001',
            ts_code='600001.SH',
            name='Member Asset',
        )
        self.second_member_asset = Asset.objects.create(
            market=self.sse,
            symbol='600002',
            ts_code='600002.SH',
            name='Second Member Asset',
        )
        self.dropped_asset = Asset.objects.create(
            market=self.szse,
            symbol='000001',
            ts_code='000001.SZ',
            name='Dropped Asset',
        ...
        IndexMembership.objects.bulk_create([
            ...
            IndexMembership(
                asset=self.second_member_asset,
                index_code='000905.SH',
                index_name='CSI 500',
                trade_date='2026-04-01',
                weight=Decimal('3.10'),
    ...
    def test_resolve_point_in_time_union_membership_uses_latest_snapshot_on_or_before_date(self):
        ...
        self.assertEqual(payload['snapshot_dates']['000905.SH'], '2026-04-15')
        self.assertEqual(payload['constituent_count'], 2)
        self.assertEqual(payload['overlap_count'], 0)
        self.assertCountEqual(
            payload['asset_ids'],
            [self.member_asset.id, self.second_member_asset.id],
        )

    def test_resolve_point_in_time_union_membership_uses_earlier_snapshot_before_rebalance(self):
        payload = resolve_point_in_time_union_membership(date(2026, 4, 10))

        self.assertEqual(payload['snapshot_dates']['000905.SH'], '2026-04-01')
        self.assertEqual(payload['constituent_count'], 3)
        self.assertCountEqual(
            payload['asset_ids'],
            [self.member_asset.id, self.second_member_asset.id, self.dropped_asset.id],
        )

    def test_resolve_point_in_time_union_membership_reports_weight_and_snapshot_per_constituent(self):
        payload = resolve_point_in_time_union_membership(date(2026, 4, 20))

        member_row = next(item for item in payload['constituents'] if item['asset_id'] == self.member_asset.id)
        self.assertCountEqual(member_row['index_codes'], ['000905.SH'])
        self.assertEqual(member_row['snapshot_dates']['000905.SH'], '2026-04-15')
        self.assertEqual(member_row['membership_weights']['000905.SH'], 4.8)
    ...
    def test_required_pit_index_codes_uses_inclusive_2010_floor(self):
        self.assertEqual(required_pit_index_codes_for_date(date(2009, 12, 31)), ())
        self.assertEqual(required_pit_index_codes_for_date(date(2010, 1, 1)), ('000905.SH',))
        self.assertEqual(required_pit_index_codes_for_date(date(2024, 9, 23)), ('000905.SH',))

    def test_ensure_pit_membership_coverage_requires_csi500_from_floor(self):
        ...
        with self.assertRaisesMessage(
            ...
            ensure_pit_membership_coverage([target_date], context='PIT test')
    ...
    def test_point_in_time_benchmark_daily_persists_internal_benchmark_rows(self):
        row = PointInTimeBenchmarkDaily.objects.create(
            benchmark_code='CSI500_PIT',
            benchmark_name='CSI 500 PIT',
            trade_date='2026-04-20',
            daily_return=Decimal('0.01234567'),
            nav=Decimal('101234.56780000'),
            constituent_count=3,
            overlap_count=0,
            metadata={'snapshot_dates': {'000905.SH': '2026-04-15'}},
        )

        self.assertEqual(str(row), 'CSI500_PIT on 2026-04-20')
...
class PointInTimeBenchmarkPrecomputeTests(TestCase):
    ...
    def test_build_point_in_time_union_benchmark_rows_uses_free_float_market_cap_weights(self):
        ...
        self.assertEqual(rows[1].overlap_count, 0)
...
class UniverseOnboardingCommandTests(TestCase):
    ...
    @patch('apps.markets.management.commands.onboard_csi500_universe.call_command')
    def test_onboard_csi500_universe_runs_expected_subcommands(self, mock_call_command):
        output = StringIO()
        with tempfile.TemporaryDirectory() as temp_dir:
            call_command(
                ...
                report_label='csi500_onboarding_test',
                report_root_dir=temp_dir,
                stdout=output,
            )

            manifest_path = Path(temp_dir) / 'csi500_onboarding_test' / 'rollout_manifest.json'
        ...
        sync_kwargs = mock_call_command.call_args_list[0].kwargs
        self.assertTrue(sync_kwargs['skip_sync_dispatch'])
        self.assertEqual(sync_kwargs['index_codes'], '000905.SH')

        benchmark_sync_kwargs = mock_call_command.call_args_list[1].kwargs
        self.assertEqual(benchmark_sync_kwargs['index_codes'], '000905.SH')

        targeted_ohlcv_kwargs = mock_call_command.call_args_list[2].kwargs
        self.assertEqual(targeted_ohlcv_kwargs['symbols'], '000001,600001')
        ...
        for call_args in mock_call_command.call_args_list[4:6]:
            self.assertEqual(call_args.kwargs['symbols'], '000001,600001')
        ...
        self.assertEqual(manifest['universe_symbols'], ['000001', '600001'])
        ...
        self.assertTrue(manifest['benchmark_output_dir'].endswith('post_onboarding'))
        self.assertIn('CSI 500 onboarding workflow complete.', output.getvalue())